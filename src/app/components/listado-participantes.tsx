...
export const ListadoParticipantes = () => {
  const { seleccionados, delSeleccionado, delIntervencion, startTime } =
    useAppStore(
      useShallow((state) => ({
        seleccionados: state.seleccionados,
        delSeleccionado: state.delSeleccionado,
        delIntervencion: state.delIntervencion,
  ...
  const processAdherente = (sel: string, index: number) => {
    delSeleccionado(sel);
    if (0 !== index) {
      delIntervencion(sel);
    }
  ...
  return (
    <>
      {seleccionados.map((p, index) => {
        ...
        classText =
          index > 1 ? "text-gray-400 text-3xl" : `text-gray-900 ${classText}`;

        return (
          <div
            className={`flex flex-col items-center justify-center sm:flex-row text-4xl cursor-pointer ${
              index === 0
                ? "mb-4 border-2 border-gray-400 p-2 rounded-md"
                : "mb-0"
            } ${index === 0 && anim ? "animate-expand" : ""} `}
            key={index}
            onClick={() => processAdherente(p, index)}
          >
